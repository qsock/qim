...
func main() {
	...
	if err := config.Init(*configFile); err != nil {
		panic(fmt.Sprintf("%v||%s", err, *configFile))
	}
	...
	if err := qjwt.Init(config.GetConfig().Jwt); err != nil {
		panic(err)
	}
	...
	if err != nil {
		panic(fmt.Sprintf("%v||%s", err, config.GetAddr()))
	}

	s := grpc.NewServer(rpc.LogMiddleware())
	passport.RegisterPassportServer(s, new(Server))
	go s.Serve(ln)
